...
#include "utils_linux_inotify_INotify.h"
...
#if DEBUG
#define CHK(id) do { \
        if ((int)utils_linux_inotify_INotify_##id != (int)id) { \
            char buffer[256]; \
            snprintf(buffer, sizeof buffer, \
                D(#id "(%#x) in C native code differs " \
                    "from class INotify definition(%#x)"), \
                id, \
                utils_linux_inotify_INotify_##id); \
 ...
 * Class:     utils_linux_inotify_INotify
 * Method:    getEvent
 * Signature: ()Lutils/linux/inotify/INotify/Event;
 */
